    ...
    bf.drop_and_create_view(conn, 'v_reporting_user', '''
        ...
        WITH usernames as (
            ...
            SELECT
                ...
                , COUNT(DISTINCT (CASE WHEN bi.bid_type = 'Manual' THEN b.bid_id ELSE NULL END)) AS manual_bids_placed
            ...
            FROM bids b
                LEFT JOIN v_bid_info bi ON bi.bid_id = b.bid_id
            GROUP BY
                username
        )
        SELECT
